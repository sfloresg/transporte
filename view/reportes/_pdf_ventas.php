...
class PDF extends FPDF
{
	...
	{                
                ...
                $this->Ln();
                $this->SetXY(9,8);
                $this->Write(0,'REPORTE DE VENTAS POR FECHA');		
		...
		$this->SetFillColor(245, 245, 245);
                ...
                $this->Cell($this->widths[3], $h, 'DESCRIPCION', $border, 0, 'C',$fill);
                $this->Cell($this->widths[4], $h, 'SERIE', $border, 0, 'C',$fill);
                $this->Cell($this->widths[5], $h, 'NUMERO', $border, 0, 'C',$fill);
                $this->Cell($this->widths[6], $h, 'NOMBRE', $border, 0, 'C',$fill);
				$this->Cell($this->widths[7], $h, 'TOTAL', $border, 0, 'C',$fill);
                $this->Ln();
...
//            }
            ...
            foreach($rowsi as $r)
            {
                ...
                $this->Cell($w[3], $h,$r['descripcion'], $border, 0, 'C', $fill);
                $this->Cell($w[4], $h,$r['serie'], $border, 0, 'C', $fill);
                $this->Cell($w[5], $h,$r['numero'], $border, 0, 'C', $fill);
				$this->Cell($w[6], $h,$r['nombre'], $border, 0, 'C', $fill);
				$this->Cell($w[7], $h,$r['total'], $border, 0, 'C', $fill);
			...
			$this->Cell($w[6], $h,"TOTAL", $border, 0, 'C', $fill);
			$this->Cell($w[7], $h,$to, $border, 0, 'C', $fill);
	}
}
...
$maxw=200;
$w = array(10,15,20,30,15,15,50,40);
